...
export const parseText = (str: string, options: ParserOptions) => {
  ...
  for (let i = 0; i < slices.length; i++) {
    if (slices[i].start > start) {
      parts.push({ str: str.slice(start, slices[i].start), formats: Array.from(activeFormats.values()) })
    }
    if (slices[i].format) {
    ...
    if (slices[i].replace) {
      parts.push({ str: str.slice(slices[i].start, slices[i].length), replace: slices[i].replace, formats: Array.from(activeFormats.values()) })
    }
